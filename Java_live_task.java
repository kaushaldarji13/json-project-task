        ...
        try (FileReader reader = new FileReader("Task-1.json")) {
            ...
            for (int i =0; i< dataList.length();i++){
                ...
                if (jsonObject.has("Query")){
                    JSONObject resultObject = jsonObject.getJSONObject("Query");
                    if (jsonObject.has("Relationships") && jsonObject.getJSONArray("Relationships").length()>0){
                        JSONObject relationship_data = jsonObject.getJSONArray("Relationships").getJSONObject(0);
                        if (relationship_data.has("Ids") && relationship_data.getJSONArray("Ids").length() > 0){

                            resultObject.put("AnswerKey",relationship_data.getJSONArray("Ids").get(0));
                            for (int j =0; j< dataList.length();j++){
                                JSONObject ansObject = dataList.getJSONObject(j);
                                if(ansObject.getString("Id").equals(resultObject.getString("AnswerKey"))){
                                    resultObject.put("answer",ansObject.getString("Text"));
